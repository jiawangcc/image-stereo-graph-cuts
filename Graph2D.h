...
enum Mode {SEGMENT=0, STEREO_GREY=1, STEREO_COLOR=2, COLOR=3};
extern float con;
extern int dp_back, dp_fore;
...
public:
	// constructors/reset
	Graph2D();
	Graph2D(Table2D<RGB> & im, double sigma);
	void Graph2D::reset(Table2D<RGB> & im, Table2D<RGB> & im2,double sigma, bool eraze_seeds, Mode mode);
	void Graph2D::GMM(Table2D<RGB> & im, Table2D<RGB> & im2,double sigma, bool eraze_seeds, Mode mode);
	...
	int compute_mincut(void (*draw_function)() = NULL);
